      ...
      <div class="page-content d-flex align-items-stretch">
        ...
        <div class="content-inner">
          ...
          <section class="dashboard-counts no-padding-bottom">
			...
			if($role == "Judge"){
				$queryProject = " SELECT * FROM Project NATURAL JOIN Rubric WHERE u_email = '$myusername' ORDER BY p_id";
			}else{
				$queryProject = "SELECT * FROM Project NATURAL JOIN Rubric ORDER BY p_id";
			}
			...
			if($rows == 0){
				echo "No projects have been scored";
			}
			else{
				while( $row = mysqli_fetch_assoc( $resultProject ) ){
					
					$myusername = $_SESSION['login_user'];
					$queryScore = "Select * FROM Rubric WHERE p_id = '$pid'";
					...
					if($role == "Judge" || $role == "Teacher"){
						...
						"<tr>
						  ...
						  <td>{$row['p_category']}</td>
						  <td>{$row['r_score']}</td>
						  <td>{$row['u_email']}</td> <td><b><input type = 'submit' name = 'Update' value = '     Update     '></b></td>
