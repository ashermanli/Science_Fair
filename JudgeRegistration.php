	<?php
	   ...
	   if($myusername == "user")
	   {
		...
		else if($role == "Teacher" || $role == "Judge")
		{
	...
	if(isset($_POST['submitted'])){
		...
		$queryInsertUser = "INSERT INTO User1 (u_email, u_name,u_role,p_id) VALUES ('$jemail', '$jname','Judge', NULL)";
		$queryInsertPass = "INSERT INTO Password (u_email, u_password) VALUES ('$jemail', '$jpass')";
		
		if((mysqli_query($db, $queryInsertUser) && mysqli_query($db, $queryInsertPass))){
	...
	if(isset($_POST['Updates'])){
		...
		$queryUpdateUser = "UPDATE User1 SET u_name = '$uname' WHERE u_email = '$uemail'";
		$queryUpdatePass = "UPDATE Password SET u_password = '$upass' WHERE u_email = '$uemail'";
		
		if((mysqli_query($db, $queryUpdateUser) && mysqli_query($db, $queryUpdatePass))){
	...
	if(isset($_POST['Delete'])){
		...
		$queryDeleteUser = "DELETE FROM User1 WHERE u_email = '$dEmail'";
		$queryDeletePass = "DELETE FROM Password WHERE u_email = '$dEmail'";
		
		if((mysqli_query($db, $queryDeleteUser) && mysqli_query($db, $queryDeletePass))){
...
$role = $_SESSION['login_role'];
			...
			<div class="content-inner">
			  ...
			  <section class="dashboard-counts no-padding-bottom">
				...
				<form action="JudgeRegistration.php" method="post">
		  ...
		  if(isset($_POST['Update'])){
			  echo " <form action='JudgeRegistration.php' method='post'>
		  <h2>Update Records</h2>
			Name:<br>
			<input type='text' name='u_name' value = '$name'><br>
			
			Email:<br>
			<input type='text' name='u_email' value = '$email'><br>
      ...
      <tbody>
        <?php
			$queryUser = "SELECT * FROM User1 NATURAL JOIN Password WHERE u_role = 'Judge'";
				...
				while( $row = mysqli_fetch_assoc( $resultUser ) ){
					...
					if($myusername == "user"){
						...
						"<tr>
						  ...
						  <td>{$row['u_name']}</td>
						  <td>{$row['u_password']}</td>
						  <td><b><input type = 'submit' name = 'Delete' value = 'Delete' formaction = 'JudgeRegistration.php'></b></td>
						  <td><b><input type = 'submit' name = 'Update' value = 'Update' formaction = 'JudgeRegistration.php'></b></td>
						  <input type='hidden' name='userEmail' value='{$row['u_email']}'>
						  <input type='hidden' name='userName' value = '{$row['u_name']}'>
						  <input type='hidden' name='userPassword' value = '{$row['u_password']}'>
						  </form>
